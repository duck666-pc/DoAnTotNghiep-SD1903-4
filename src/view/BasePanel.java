...
public abstract class BasePanel<T> extends javax.swing.JPanel {
    ...
    protected String getEntityName() {
        ...
        if (n.contains("QLKH")) return "khách hàng";
        if (n.contains("QLNL")) return "nguyên liệu";
        if (n.contains("QLNV")) return "nhân viên";
        if (n.contains("QLSP")) return "sản phẩm";
        return "đối tượng";
    }
    ...
    public final void initTable() {
        ...
        if (jTable == null) throw new IllegalStateException("jTable is null. Ensure UI is initialized before calling initTable()");
    ...
    public void fillTable() {
        ...
        try {
            for (T entity : getAllEntities()) addEntityToTable(entity);
        } catch (Exception ex) { showError(ex, "Lỗi khi tải dữ liệu: "); }
    }
    ...
    public void searchById(String id) {
        ...
        try {
            ...
            if (!found) showMessage("Không tìm thấy với ID: " + id);
        } catch (Exception ex) { showError(ex, "Lỗi khi tải dữ liệu: "); }
    }
    ...
    protected void handleAddAction() {
        ...
        try {
            ...
            fillTable();
            clearForm();
        } catch (Exception ex) { showError(ex, "Lỗi: "); }
    }

    protected void handleDeleteAction() {
        if (currentRow < 0) { showMessage("Vui lòng chọn một dòng để xóa!"); return; }
    ...
    protected void handleUpdateAction() {
        if (currentRow < 0) { showMessage("Vui lòng chọn một dòng để sửa!"); return; }
        if (!validateForm()) return;
